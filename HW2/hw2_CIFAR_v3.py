...
def multi_route_model():
    ...
    # right route
    x2 = AveragePooling2D(pool_size = (2, 2))(input_1)
    ...
    x1 = CNN_Layer(x1, 256)
    # right route
    x2 = Conv2D(256, kernel_size=(1, 1), strides=1, padding='same', kernel_initializer='he_normal', kernel_regularizer=l2(1e-4))(merge2)
    # merge 2 conv outputs
    ...
    merge3 = Subtract()([x1, x2])
    merge3 = ReLU()(merge3)
    output7 = Decide_block(merge3, 10, kernel_size=8)
    ...
    x = Concatenate()([output1, output2, output4, output5, output7])
    x = Dropout(0.25)(x)
    x = Dense(num_classes, activation='softmax', kernel_initializer='he_normal')(x)
    output = x
...
# optimizers = SGD(learning_rate=0.1, momentum=0.9, decay=0.001)
...
if mode == 'train':
    ...
    adapt_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.2, patience=7, verbose=1, min_lr=0.00001)
...
# elif mode == 'predict':
    ...
    img = np.expand_dims(cv2.imread('run/CIFAR/0.jpg', cv2.IMREAD_GRAYSCALE), axis=-1)
